...
rsVisi.Source = "SELECT ID,Del,  Head, Content, updateby, tgl   FROM dbo.Visi_Misi";
rsVisi.CursorType = 0;
...
rsVisi_numRows += Repeat1__numRows;
%>
<% var MM_paramName = ""; %>
<%
    ...
    <TR> 
      <TD width=1000 valign="top" class=tdtitle> <!-- InstanceBeginEditable name="EditRegion3" -->
        <TABLE cellSpacing=0 cellPadding=2 width="100%" border=0><TBODY>
          ...
          <TD class=tdtitle>Browse Visi &amp; Mision</TD>
          </TR>
          <TR vAlign=top> 
            ...
            <TABLE cellSpacing=1 cellPadding=2 width="100%" border=0>
              <FORM action=Add_visi.asp method=post>
                <TBODY>
                  <TR> 
                  ...
                  <% while ((Repeat1__numRows-- != 0) && (!rsVisi.EOF)) { %>
                  <TR class=trlight> 
                    <TD vAlign=top><div align="center"><%=response.write(Repeat1__index + 1) %></div></TD>
                    <TD vAlign=top align=middle><A HREF="Del_visi.asp?<%= MM_keepNone + ((MM_keepNone!="")?"&":"") + "ID=" + rsVisi.Fields.Item("ID").Value %>"><%=(rsVisi.Fields.Item("Del").Value)%></A> </TD>
                    <TD vAlign=top align=middle><A HREF="M_visi.asp?<%= MM_keepNone + ((MM_keepNone!="")?"&":"") + "ID=" + rsVisi.Fields.Item("ID").Value %>"><IMG 
                  height=15 
                  src="modify.gif" 
                  width=21 border=0></A> </TD>
                    <TD vAlign=top><%=(rsVisi.Fields.Item("Head").Value)%></TD>
                    <TD vAlign=top><%=(rsVisi.Fields.Item("Content").Value)%></TD>
                    <TD vAlign=top><%=(rsVisi.Fields.Item("updateby").Value)%></TD>
                    <TD vAlign=top><%= DoDateTime((rsVisi.Fields.Item("tgl").Value), 1, 2057) %></TD>
                  </TR>
                  <%
  Repeat1__index++;
  rsVisi.MoveNext();
