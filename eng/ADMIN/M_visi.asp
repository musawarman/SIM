...
if (String(Request("MM_update")) == "form1" &&
  ...
  var MM_editTable  = "dbo.Visi_Misi";
  var MM_editColumn = "ID";
  var MM_recordId = "" + Request.Form("MM_recordId") + "";
  var MM_editRedirectUrl = "br_Visi.asp";
  var MM_fieldsStr = "textarea2|value|hiddenField|value|select|value";
  var MM_columnsStr = "Content|',none,''|updateby|',none,''|Lang|',none,''";
...
%>
<%
var rsVisi__MMColParam = "1";
if (String(Request.QueryString("ID")) != "undefined" && 
    String(Request.QueryString("ID")) != "") { 
  rsVisi__MMColParam = String(Request.QueryString("ID"));
}
%>
<%
var rsVisi = Server.CreateObject("ADODB.Recordset");
rsVisi.ActiveConnection = MM_DBConn_STRING;
rsVisi.Source = "SELECT *  FROM dbo.Visi_Misi  WHERE ID = "+ rsVisi__MMColParam.replace(/'/g, "''") + "";
rsVisi.CursorType = 0;
    ...
    <TR> 
      <TD width=1000 valign="top" class=tdtitle> <!-- InstanceBeginEditable name="EditRegion3" --> 
        <TABLE border=0 cellPadding=2 cellSpacing=0 width="100%"><TBODY>
          <TR> 
            <TD class=tdtitle>Modify Vision &amp; Mission</TD>
          </TR>
          <TR vAlign=top> 
                ...
                <TABLE border=0 cellPadding=2 cellSpacing=1 width="100%">
                  <TBODY>
                    <TR> 
                      ...
                      <TD valign="top" class=tdlight> <textarea name="textarea" cols="30" rows="5"><%=(rsVisi.Fields.Item("Head").Value)%></textarea> 
                      </TD>
                    </TR>
                    <TR> 
                      <TD align=right class=tddark vAlign=top>Content * </TD>
                      <TD class=tddark vAlign=top>:</TD>
                      <TD class=tdlight><textarea name="textarea2" cols="40" rows="10"><%=(rsVisi.Fields.Item("Content").Value)%></textarea> 
                      </TD>
                    </TR>
                    <TR> 
                      <TD align=right class=tddark>&nbsp;</TD>
                      <TD class=tddark>&nbsp;</TD>
                    ...
                    <TR> 
                      ...
                      <TD class=tdlight vAlign=top> <select name="select">
                          <option value="Indonesia" <%=(("Indonesia" == (rsVisi.Fields.Item("Lang").Value))?"SELECTED":"")%>>Indonesia</option>
                          <option value="Inggris" <%=(("Inggris" == (rsVisi.Fields.Item("Lang").Value))?"SELECTED":"")%>>Inggris</option>
                          <% 
while (!rsVisi.EOF) {
%>
                          <option value="<%=(rsVisi.Fields.Item("Lang").Value)%>" <%=((rsVisi.Fields.Item("Lang").Value == (rsVisi.Fields.Item("Lang").Value))?"SELECTED":"")%> ><%=(rsVisi.Fields.Item("Lang").Value)%></option>
                          <%
  rsVisi.MoveNext();
}
if (rsVisi.CursorType > 0) {
  if (!rsVisi.BOF) rsVisi.MoveFirst();
} else {
  rsVisi.Requery();
}
%>
                ...
                <input type="hidden" name="MM_recordId" value="<%= rsVisi.Fields.Item("ID").Value %>">
              </form>
