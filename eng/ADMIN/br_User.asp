...
rsUser.Source = "SELECT memberID, Del, DeptID, Name, Login, Password, updateby, LevelID,  tgl  FROM dbo.members  WHERE DeptID = 'ADM'";
rsUser.CursorType = 0;
...
rsUser_numRows += Repeat1__numRows;
%>
<% var MM_paramName = ""; %>
<%
    ...
    <TR> 
      <TD width=1000 valign="top" class=tdtitle> <!-- InstanceBeginEditable name="EditRegion3" --> 
        <TABLE cellSpacing=0 cellPadding=2 width="100%" border=0><TBODY>
          <TR> 
            <TD class=tdtitle>Browse User</TD>
          </TR>
          <TR vAlign=top> 
                ...
                <FORM action=Add_User.asp method=post>
                  <TBODY>
                    <TR> 
                      ...
                      <TD class=tdbrowsetitle width=25>&nbsp;</TD>
                      <TD class=tdbrowsetitle width=200>Name</TD>
                      <TD class=tdbrowsetitle width=200>User Name</TD>
                      <TD class=tdbrowsetitle width=150>Post By</TD>
                      <TD class=tdbrowsetitle width=150>Grade</TD>
                    </TR>
                    <% while ((Repeat1__numRows-- != 0) && (!rsUser.EOF)) { %>
                    <TR class=trlight> 
                      <TD vAlign=top><div align="left"><%=(Repeat1__index + 1)%></div></TD>
                      <TD vAlign=top align=middle><A HREF="Del_User.asp?<%= MM_keepNone + ((MM_keepNone!="")?"&":"") + "memberID=" + rsUser.Fields.Item("memberID").Value %>"><%=(rsUser.Fields.Item("Del").Value)%></A> </TD>
                      <TD vAlign=top align=middle><A HREF="M_User.asp?<%= MM_keepNone + ((MM_keepNone!="")?"&":"") + "memberID=" + rsUser.Fields.Item("memberID").Value %>"><IMG 
                  src="modify.gif" 
                  width=21 
                  height=15 border=0></A> </TD>
                      <TD vAlign=top><%=(rsUser.Fields.Item("Name").Value)%></TD>
                      <TD vAlign=top><%=(rsUser.Fields.Item("login").Value)%></TD>
                      <TD vAlign=top><%=(rsUser.Fields.Item("updateby").Value)%></TD>
                      <TD vAlign=top><%=(rsUser.Fields.Item("levelID").Value)%></TD>
                    </TR>
                    <%
  Repeat1__index++;
  rsUser.MoveNext();
