...
rsSejarah.Source = "SELECT ID,del,head,content,updateby,tgl  FROM dbo.Sejarah";
...
rsSejarah_numRows += Repeat1__numRows;
%>
<% var MM_paramName = ""; %>
<%
  ...
  <TBODY>
    <TR> 
      ...
      <TABLE cellSpacing=0 cellPadding=2 width="100%" border=0><TBODY>
          ...
          <TD class=tdtitle>Browse History &amp; Background</TD>
          </TR>
          <TR vAlign=top> 
            <TD class=tdverylight> <TABLE cellSpacing=1 cellPadding=2 width="100%" border=0>
              ...
              <TABLE cellSpacing=1 cellPadding=2 width="100%" border=0>
                <FORM action=Add_sejarah.asp method=post>
                  <TBODY>
                    <TR> 
                  ...
                  <% while ((Repeat1__numRows-- != 0) && (!rsSejarah.EOF)) { %>
                  <TR class=trlight> 
                    <TD vAlign=top><div align="center"><%=response.write(Repeat1__index + 1) %></div></TD>
                    <TD vAlign=top align=middle><A HREF="Del_sejarah.asp?<%= MM_keepNone + ((MM_keepNone!="")?"&":"") + "ID=" + rsSejarah.Fields.Item("ID").Value %>"><%=(rsSejarah.Fields.Item("Del").Value)%></A> </TD>
                    <TD vAlign=top align=middle><A HREF="M_Sejarah.asp?<%= MM_keepNone + ((MM_keepNone!="")?"&":"") + "ID=" + rsSejarah.Fields.Item("ID").Value %>"><IMG 
                  height=15 
                  src="modify.gif" 
                  width=21 border=0></A> </TD>
                    <TD vAlign=top><%=(rsSejarah.Fields.Item("Head").Value)%></TD>
                    <TD vAlign=top><%=(rsSejarah.Fields.Item("Content").Value)%></TD>
                    <TD vAlign=top><%=(rsSejarah.Fields.Item("updateby").Value)%></TD>
                    <TD vAlign=top><%= DoDateTime((rsSejarah.Fields.Item("tgl").Value), 1, 2057) %></TD>
                  </TR>
                  <%
  Repeat1__index++;
  rsSejarah.MoveNext();
