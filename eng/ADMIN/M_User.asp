...
if (String(Request("MM_update")) == "form1" &&
  ...
  var MM_fieldsStr = "p_title|value|p_title3|value|p_title4|value|select|value|hiddenField|value";
  var MM_columnsStr = "Name|',none,''|Login|',none,''|Password|',none,''|levelID|none,none,NULL|updateby|',none,''";
...
rsUser.Source = "SELECT memberID, DeptID, Name, Login, Password, levelID FROM dbo.members WHERE memberID = "+ rsUser__MMColParam.replace(/'/g, "''") + "";
rsUser.CursorType = 0;
    ...
    <TR> 
      <TD width=1000 valign="top" class=tdtitle> <!-- InstanceBeginEditable name="EditRegion3" --> 
        <TABLE border=0 cellPadding=2 cellSpacing=0 width="100%"><TBODY>
          <TR> 
            <TD class=tdtitle>Modify User</TD>
          </TR>
          <TR vAlign=top> 
            <TD class=tdverylight> <form action="<%=MM_editAction%>" method="POST" name="form1">
                <TABLE border=0 cellPadding=2 cellSpacing=1 width="100%">
                  <TBODY>
                    <TR> 
                      <TD align=right class=tddark width=125>User Name *</TD>
                      <TD class=tddark width=5>:</TD>
                      <TD class=tdlight vAlign=top> <INPUT 
                  name=p_title class=btn_text value="<%=(rsUser.Fields.Item("Name").Value)%>" size=40> 
                      </TD>
                    </TR>
                    <TR> 
                      <TD align=right class=tddark vAlign=top>Login * </TD>
                      <TD class=tddark vAlign=top>:</TD>
                      <TD class=tdlight> <INPUT 
                  name=p_title3 class=btn_text value="<%=(rsUser.Fields.Item("Login").Value)%>" size=40> 
                      </TD>
                    </TR>
                    <TR> 
                      <TD align=right class=tddark vAlign=top>Password * </TD>
                      <TD class=tddark vAlign=top>:</TD>
                      <TD class=tdlight><INPUT 
                  name=p_title4 class=btn_text value="<%=(rsUser.Fields.Item("Password").Value)%>" size=40> 
                      </TD>
                    </TR>
                    <TR> 
                      <TD align=right class=tddark vAlign=top>Grade&nbsp;</TD>
                      <TD class=tddark vAlign=top>:</TD>
                      <TD class=tdlight vAlign=top> <select name="select">
                          <option value="7" <%=((7 == (rsUser.Fields.Item("memberID").Value))?"SELECTED":"")%>>Administrator</option>
                          <option value="6" <%=((6 == (rsUser.Fields.Item("memberID").Value))?"SELECTED":"")%>>Marketing</option>
                          <option value="5" <%=((5 == (rsUser.Fields.Item("memberID").Value))?"SELECTED":"")%>>HRD</option>
                          <option value="4" <%=((4 == (rsUser.Fields.Item("memberID").Value))?"SELECTED":"")%>>Operator</option>
                          <% 
while (!rsUser.EOF) {
%>
                          <option value="<%=(rsUser.Fields.Item("levelID").Value)%>" <%=((rsUser.Fields.Item("levelID").Value == (rsUser.Fields.Item("memberID").Value))?"SELECTED":"")%> ><%=(rsUser.Fields.Item("levelID").Value)%></option>
                          <%
  rsUser.MoveNext();
}
if (rsUser.CursorType > 0) {
  if (!rsUser.BOF) rsUser.MoveFirst();
} else {
  rsUser.Requery();
}
%>
