...
%>
<%
var rsontact = Server.CreateObject("ADODB.Recordset");
rsontact.ActiveConnection = MM_DBConn_STRING;
rsontact.Source = "SELECT ID, Del, Company, Address1, Address2, City, Phone, Fax, UpdateBy, tgl  FROM dbo.Contact";
rsontact.CursorType = 0;
rsontact.CursorLocation = 2;
rsontact.LockType = 1;
rsontact.Open();
var rsontact_numRows = 0;
%>
<%
var Repeat1__numRows = -1;
var Repeat1__index = 0;
rsontact_numRows += Repeat1__numRows;
%>
<% var MM_paramName = ""; %>
<%
  ...
  <TBODY>
    <TR> 
      ...
      <TABLE cellSpacing=0 cellPadding=2 width="100%" border=0><TBODY>
          ...
          <TD class=tdtitle>Browse Contact Us</TD>
          </TR>
          <TR vAlign=top> 
            <TD class=tdverylight> <TABLE cellSpacing=1 cellPadding=2 width="100%" border=0>
              ...
              <TABLE cellSpacing=1 cellPadding=2 width="100%" border=0>
                <FORM action=add_contact.asp method=post>
                  <TBODY>
                    <TR> 
                  ...
                  <% while ((Repeat1__numRows-- != 0) && (!rsontact.EOF)) { %>
                  <TR class=trlight> 
                    <TD vAlign=top><div align="center"><%=response.write(Repeat1__index + 1) %></div></TD>
                    <TD vAlign=top align=middle><A HREF="Del_contact.asp?<%= MM_keepNone + ((MM_keepNone!="")?"&":"") + "ID=" + rsontact.Fields.Item("ID").Value %>"><%=(rsontact.Fields.Item("Del").Value)%></A> </TD>
                    <TD vAlign=top align=middle><A HREF="M_contact.asp?<%= MM_keepNone + ((MM_keepNone!="")?"&":"") + "ID=" + rsontact.Fields.Item("ID").Value %>"><IMG 
                  height=15 
                  src="modify.gif" 
                  width=21 border=0></A> </TD>
                    <TD vAlign=top><%=(rsontact.Fields.Item("Company").Value)%></TD>
                    <TD vAlign=top><%=(rsontact.Fields.Item("Address1").Value)%></TD>
                    <TD vAlign=top><%=(rsontact.Fields.Item("UpdateBy").Value)%></TD>
                    <TD vAlign=top><%= DoDateTime((rsontact.Fields.Item("tgl").Value), 1, 2057) %></TD>
                  </TR>
                  <%
  Repeat1__index++;
  rsontact.MoveNext();
